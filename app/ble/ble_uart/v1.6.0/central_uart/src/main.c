...
int bt_nus_handles_assign_fixed(struct bt_conn *conn,
			  struct bt_nus_client *nus_c)
{

	LOG_INF("Assign fixed handles for NUS service.");
	memset(&nus_c->handles, 0xFF, sizeof(nus_c->handles));

	/* NUS TX */
	nus_c->handles.tx = 0x12;
	/* NUS TX CCC */
	nus_c->handles.tx_ccc = 0x13;

	/* NUS RX */
	nus_c->handles.rx = 0x15;
...
static void assign_service_handles(struct bt_conn *conn, struct bt_nus_client *nus)
{  
    bt_nus_handles_assign_fixed(conn, nus);
	bt_nus_subscribe_receive(nus);
    LOG_INF("Service assign completed");
}
...
static void connected(struct bt_conn *conn, uint8_t conn_err)
{
    ...
    //LOG_INF("conn handle, type, role-> (%d), (%d), (%d)", conn->handle, conn->type, conn->role);
	/* err = bt_gatt_exchange_mtu(conn, &exchange_params);
	if (err) {
		LOG_WRN("MTU exchange failed (err %d)", err);
	}
    */ /*  Disable MTU update here. */
	err = bt_conn_set_security(conn, BT_SECURITY_L2);
	if (err) {
		LOG_WRN("Failed to set security: %d", err);

		//gatt_discover(conn); /* disable gatt discovery, and manual assign handles */
	}

    assign_service_handles(conn,  &nus_client);
			     ...
			     enum bt_security_err err)
{
	...
	//gatt_discover(conn); /*  disable discover, manual assign handles */
}
